...
void loop() {
  ...
  t2 = millis();
  t3 = t2 - t1;
  Serial.println("LR configuration successfully turned on in time: " + t3);
  delay(500); //can remove later once we know the minimum deadtime and can ensure that it is being taken care of
  t1 = millis();
  RL(on_time_RL);
  t2 = millis();
  t3 = t2 - t1;
  Serial.println("RL configuration successfully turned on in time: " + t3);
  delay(500); //same as above
}
